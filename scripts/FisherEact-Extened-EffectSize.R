...
cohen_d <- function(df1, df2) {
  n1 <- length(df1)
  n2 <- length(df2)
  mean_x <- mean(df1, na.rm = TRUE)
  mean_y <- mean(df2, na.rm = TRUE)
  sd_pooled <- sqrt(((n1 - 1) * var(df1, na.rm = TRUE) + (n2 - 1) * var(df2, na.rm = TRUE)) / (n1 + n2 - 2))
  d <- (mean_x - mean_y) / sd_pooled
  
  return(d)
}

# 新添加的计算效应量函数（Cohen's d）
compute_effect_size <- function(df1, df2) {
  # 将数据框转为数值向量，假定 df1 和 df2 是一行数据框
  vec1 <- as.numeric(unlist(df1))
  vec2 <- as.numeric(unlist(df2))
  
  n1 <- length(vec1)
  n2 <- length(vec2)
  mean1 <- mean(vec1, na.rm = TRUE)
  mean2 <- mean(vec2, na.rm = TRUE)
  pooled_sd <- sqrt(((n1 - 1) * var(vec1, na.rm = TRUE) + (n2 - 1) * var(vec2, na.rm = TRUE)) / (n1 + n2 - 2))
...
main <- function() {
  ...
  for (pair in cross_pairs) {
    ...
    cat(sprintf("    效应量 (Cohen's d): %s\n", ifelse(is.na(effect_size), "NA", sprintf("%.4f", effect_size))))
    cat(sprintf("    解释：%s\n", interpretation))
